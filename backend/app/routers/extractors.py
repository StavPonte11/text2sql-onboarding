...
def create_extractor(
    payload: HttpExtractorCreate, session: Session = Depends(get_session)
):
    """Register a new HTTP extractor."""
    # Check if name exists
    existing = session.exec(
        select(HttpExtractor).where(HttpExtractor.name == payload.name)
    ).first()
    if existing:
        raise HTTPException(
            status_code=400, detail="Extractor with this name already exists"
        )
