...
def _analyze_column(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as inner_exec:
        f_dist = inner_exec.submit(execute_with_timeout, build_distinct_count_query(fqn, col_name), table_id)
        f_null = inner_exec.submit(execute_with_timeout, build_null_ratio_query(fqn, col_name), table_id)
...
def run_table_profiling(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(30, len(columns_meta) or 1)) as worker_executor:
        ...
        for col_name, data_type in columns_meta:
            futures.append(worker_executor.submit(_analyze_column, fqn, table_id, col_name, data_type, result.row_count))
