    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
        ...
        <span
          style={{
            fontWeight: 600,
            color: count > 0 ? 'var(--status-degraded)' : 'var(--text-muted)',
          }}
        >
          {count}
        </span>
...
export function HealthDashboard({ tableId }: { tableId: string }) {
  ...
  const maxFailure = Math.max(
    health.failure_wrong_table,
    health.failure_wrong_sql,
    health.failure_empty_result,
    health.failure_execution_error,
    1,
  );
  const statusIcon =
    health.health_status === 'good' ? (
      <CheckCircle size={16} color="var(--status-production)" />
    ) : health.health_status === 'warning' ? (
      <AlertTriangle size={16} color="var(--status-sandbox)" />
    ) : (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {[
          {
            ...
            value:
              health.eval_success_rate != null
                ? `${Math.round(health.eval_success_rate * 100)}%`
                : '—',
          ...
          },
          {
            ...
            value:
              health.feedback_ratio != null
                ? `${Math.round(health.feedback_ratio * 100)}% 👍`
                : '—',
          ...
          },
          {
            label: 'Data Quality',
            value:
              health.data_quality_score != null
                ? `${Math.round(health.data_quality_score * 100)}%`
                : '—',
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <FailureBar
            label="Wrong Table Selected"
            count={health.failure_wrong_table}
            max={maxFailure}
          />
          <FailureBar
            label="Wrong / Invalid SQL"
            count={health.failure_wrong_sql}
            max={maxFailure}
          />
          <FailureBar label="Empty Result" count={health.failure_empty_result} max={maxFailure} />
          <FailureBar
            label="Execution Error"
            count={health.failure_execution_error}
            max={maxFailure}
          />
        </div>
        {health.failure_wrong_table +
