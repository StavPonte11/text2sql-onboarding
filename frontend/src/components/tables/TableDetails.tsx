...
import { ArrowLeft, CornerUpLeft, FlaskConical, Rocket, ShieldCheck } from 'lucide-react';

import { enrichmentApi, evalApi, tablesApi } from '../../api/client';
...
export function TableDetails() {
  ...
  const currentBatchRegressions =
    batchRuns?.filter((r) => r.triggered_by === 'promotion-regression') || [];
  ...
  const isRecentPromotion =
    latestPromotion && dayjs().diff(dayjs(latestPromotion.created_at), 'minute') < 5;
  const isPromoting =
    !!activePromotion ||
    activeRegressions.length > 0 ||
    (isRecentPromotion && currentBatchRegressions.length > 0);

  const phase = activePromotion
    ? 'Phase 1: Evaluating Target Table'
    : activeRegressions.length > 0
      ? 'Phase 2: Running Regression Suite'
      : 'Promotion Complete';
      ...
      {isPromoting && (
        ...
        >
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              {!activePromotion && currentBatchRegressions.length > 0 && (
                <span style={{ fontSize: 11, opacity: 0.8 }}>
                  {
                    currentBatchRegressions.filter(
                      (r) => r.status === 'completed' || r.status === 'failed',
                    ).length
                  }{' '}
                  / {currentBatchRegressions.length} Production Tables Validated
                </span>
              )}
            </div>
          </div>
          <button
            ...
            onClick={() =>
              activePromotion ? setActiveTab('evaluations') : setShowRegressionModal(true)
            }
          >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div className="card">
              ...
              {[
                ...
                {
                  label: 'Last Updated',
                  value: dayjs(table.updated_at).format('MMM D, YYYY HH:mm'),
                },
              ].map(({ label, value }) => (
                <div
                  key={label}
                  ...
                  <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div
                      style={{
                        ...
                        fontSize: 11,
                        fontWeight: 700,
                        color: table.status === s ? 'var(--accent-hover)' : 'var(--text-muted)',
                      }}
                    >
                      {table.status === s ? '●' : '○'}
                    </div>
        ...
        <div className="modal-overlay" onClick={() => setShowRegressionModal(false)}>
          <div className="modal" style={{ maxWidth: 600 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal__actions">
              <button className="btn btn--primary" onClick={() => setShowRegressionModal(false)}>
                Close
              </button>
            </div>
          </div>
