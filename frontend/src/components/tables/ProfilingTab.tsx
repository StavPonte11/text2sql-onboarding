...
import { BarChart2, RefreshCw, Link2, AlertTriangle, ChevronDown, Layers, Maximize2, LayoutGrid, AlignLeft } from "lucide-react";
...
function buildNumericBins(topValues: { value: string | number, count: number }[], totalRows?: number, nullCount?: number, numBins = 8) {
  let nanCount = 0;
  ...
  let bins: any[] = [];
  if (pairs.length > 0) {
    const minV = pairs[0].v, maxV = pairs[pairs.length - 1].v;
    if (minV === maxV) {
      bins = [{ label: fmtNum(minV), range: String(minV), count: pairs.reduce((s, x) => s + x.count, 0) }];
    } else {
      const step = (maxV - minV) / numBins;
      const rawBins = Array.from({ length: numBins }, (_, i) => ({ lo: minV + i * step, hi: minV + (i + 1) * step, count: 0 }));
      ...
      bins = rawBins.filter((b: any) => b.count > 0).map((b: any) => ({ label: fmtNum(b.lo), range: `${fmtNum(b.lo)} – ${fmtNum(b.hi)}`, count: b.count }));
    }
  }
...
function buildDateBins(topValues: { value: string | number, count: number }[], totalRows?: number, nullCount?: number, numBins = 8) {
  let nanCount = 0;
  ...
  let bins: any[] = [];
  if (pairs.length > 0) {
    const tMin = pairs[0].t, tMax = pairs[pairs.length - 1].t;
    if (tMin === tMax) {
      bins = [{ label: fmtDate(tMin / 1000), range: fmtDate(tMin / 1000), count: pairs.reduce((s, x) => s + x.count, 0) }];
    } else {
      const step = (tMax - tMin) / numBins;
      const rawBins = Array.from({ length: numBins }, (_, i) => ({ lo: tMin + i * step, hi: tMin + (i + 1) * step, count: 0 }));
      ...
      bins = rawBins.filter((b: any) => b.count > 0).map((b: any) => ({ label: fmtDate(b.lo / 1000), range: `${fmtDate(b.lo / 1000)} – ${fmtDate(b.hi / 1000)}`, count: b.count }));
    }
  }
...
function NestedFieldExplorer({ rootChildren, rootDataType }: { rootChildren: RowField[], rootDataType?: string }) {
  ...
  const DOT_COLOR: Record<string, string> = {
    time: "#a78bfa", continuous: "#60a5fa", categorical: "#34d399",
    geo: "#f59e0b", boolean: "#ec4899", row: "#f97316", complex: "#6b7280",
  };

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
      {rootDataType && (
        <div style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "monospace", marginBottom: 10, opacity: 0.8 }}>
      ...
      {rootChildren.map((f, i) => {
        ...
        const isRow = f.semantic_type === "row" || f.semantic_type === "complex" || (f.stats?.children && f.stats.children.length > 0);
        ...
        return (
          <div key={i} style={{
            ...
            <div
              ...
              onMouseEnter={e => { if (!isRow) (e.currentTarget as HTMLDivElement).style.background = "var(--bg-base)"; }}
            ...
            >
              ...
              <span style={{ fontWeight: 600, fontSize: 14, color: "var(--text)", flex: "0 0 auto" }}>{f.name}</span>
              <span style={{ fontSize: 11, fontFamily: "monospace", background: "var(--bg-base)", padding: "1px 6px", borderRadius: 4, color: "var(--text-muted)", border: "1px solid var(--border)" }}>
                {f.data_type}
              </span>
              ...
              {!isExpanded && (
                <div style={{ marginLeft: "auto", display: "flex", gap: 16, fontSize: 12, color: "var(--text-muted)" }}>
                  {isRow && (
                    <span>{(f.stats?.children ?? []).length} fields</span>
                  )}
                  {!isRow && f.null_rate !== undefined && (
                    <span>
                      <span style={{ color: f.null_rate > 0.1 ? "var(--status-degraded)" : "var(--status-production)" }}>
                        {pct(f.null_rate)}
                      </span> nulls
                    </span>
                  )}
                  {!isRow && f.distinct_count !== undefined && <span>{fmt(f.distinct_count)} unique</span>}
                  {!isRow && (f.semantic_type === "time" || f.is_time) && f.min_value && f.max_value && (
                    <span>{fmtDate(f.min_value)} → {fmtDate(f.max_value)}</span>
                  )}
                  {!isRow && f.semantic_type === "continuous" && f.min_value && f.max_value && (
                    <span>{f.min_value} – {f.max_value}</span>
                  )}
                  {!isRow && f.top_values && f.top_values.length > 0 && f.semantic_type === "categorical" && (
...
function MiniMetric({ col, totalRows }: { col: ColumnProfile, totalRows?: number }) {
  ...
  if (isRow) return <div style={{ fontSize: 12, color: "var(--text-muted)", fontStyle: "italic", marginTop: 8 }}>Nested object</div>;
      ...
      <div style={{ marginTop: 8, fontSize: 11, color: "var(--text-muted)" }}>
        <div style={{ height: 24 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={[{ v: 0 }, { v: 1 }]}><YAxis hide /><Area type="monotone" dataKey="v" stroke="var(--accent-hover)" fill="var(--accent-hover)" fillOpacity={0.2} strokeWidth={2} isAnimationActive={false} /></AreaChart>
          </ResponsiveContainer>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
          <span>{fmtDate(col.min_value)}</span><span>{fmtDate(col.max_value)}</span>
        </div>
      </div>
    );
  }
  ...
  if (col.data_type?.toLowerCase() === "boolean" && col.top_values) {
    ...
    if (total > 0) return (
      <div style={{ display: "flex", gap: 2, height: 14, marginTop: 10 }}>
        <div style={{ flex: t, background: "var(--status-production)", borderRadius: "2px 0 0 2px" }} title={`True: ${pct(t / total)}`} />
        <div style={{ flex: f, background: "var(--status-degraded)" }} title={`False: ${pct(f / total)}`} />
        {(otherCount > 0 || nulls > 0) && <div style={{ flex: otherCount + nulls, background: "var(--bg-base)", borderRadius: "0 2px 2px 0" }} title={`Other/Null: ${pct((otherCount + nulls) / total)}`} />}
      </div>
    );
  }
    ...
    const bins = col.stats_json?.histogram 
      ...
      : col.top_values && col.top_values.length > 0
        ...
        : [
          ...
          { label: '', range: '', count: (Number(col.stats_json?.q25) || 0) - (Number(col.min_value) || 0) },
          { label: '', range: '', count: (Number(col.stats_json?.median) || 0) - (Number(col.stats_json?.q25) || 0) },
          { label: '', range: '', count: (Number(col.stats_json?.q75) || 0) - (Number(col.stats_json?.median) || 0) },
          { label: fmtNum(Number(col.max_value) || 0), range: '', count: (Number(col.max_value) || 0) - (Number(col.stats_json?.q75) || 0) },
        ].filter(b => b.count > 0);
    return (
  ...
  if (col.top_values && col.top_values.length > 0) {
    ...
    const bars = [
      ...top3.map(tv => ({ name: String(tv.value).length > 8 ? String(tv.value).slice(0, 7) + '…' : String(tv.value), count: tv.count, full: String(tv.value) })),
      ...
      <div style={{ height: 44, marginTop: 8 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={bars} margin={{ top: 2, right: 0, left: 0, bottom: 0 }}>
            <XAxis dataKey="name" tick={{ fontSize: 8, fill: 'var(--text-muted)' }} tickLine={false} axisLine={false} interval={0} />
            <Tooltip cursor={{ fill: 'var(--bg-base)' }} content={({ active, payload }) => active && payload?.length ? <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 8px', fontSize: 11 }}><div style={{ color: 'var(--text-muted)' }}>{payload[0].payload.full}</div><div style={{ fontWeight: 600 }}>{fmt(payload[0].payload.count)} ({pct(payload[0].payload.count / (total || 1))})</div></div> : null} />
            <Bar dataKey="count" fill="var(--accent-hover)" radius={[2, 2, 0, 0]} isAnimationActive={false} />
          </BarChart>
        </ResponsiveContainer>
      </div>
  ...
  if (col.distinct_count) {
    return <div style={{ fontSize: 18, fontWeight: 700, color: "var(--text)", marginTop: 8 }}>{fmt(col.distinct_count)}<span style={{ fontSize: 11, fontWeight: 400, color: "var(--text-muted)", marginLeft: 4 }}>unique</span></div>;
  }
...
function ColumnReportRow({ col, isNested = false, totalRows }: { col: ColumnProfile, isNested?: boolean, totalRows?: number }) {
  const isRow = col.semantic_type === "row" || col.semantic_type === "complex" || col.data_type?.toLowerCase().includes("row") || col.data_type?.toLowerCase().includes("array") || col.data_type?.toLowerCase().includes("map") || col.data_type?.toLowerCase().includes("json") || (col.stats_json?.children && col.stats_json.children.length > 0);
          ...
          {!isRow && (
            <div style={{ marginTop: 24 }}>
              {(() => {
                ...
                const tooltipStyle = { background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', fontSize: 12 };
                    ...
                    <div style={{ height: 160 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={bins} margin={{ top: 8, right: 12, left: 0, bottom: 36 }}>
                          <XAxis dataKey="label" tick={{ fontSize: 10, fill: 'var(--text-muted)' }} tickLine={false} axisLine={{ stroke: 'var(--border)' }} angle={-35} textAnchor="end" interval={0} />
                          <YAxis tick={{ fontSize: 10, fill: 'var(--text-muted)' }} tickLine={false} axisLine={{ stroke: 'var(--border)' }} width={48} tickFormatter={v => fmt(v) ?? ''} hide={!hasDatums} />
                          <Tooltip cursor={{ fill: 'var(--bg-base)' }} content={({ active, payload }) => active && payload?.length ? <div style={tooltipStyle}><div style={{ color: 'var(--text-muted)' }}>{payload[0].payload.range}</div>{hasDatums && <div style={{ fontWeight: 600, marginTop: 4 }}>{fmt(payload[0].payload.count)} values</div>}</div> : null} />
                          <Bar dataKey="count" fill="var(--accent-hover)" radius={[4, 4, 0, 0]} isAnimationActive={false} maxBarSize={40} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  );
                    ...
                    <div style={{ height: 160 }}>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={bins} margin={{ top: 8, right: 12, left: 0, bottom: 40 }}>
                          <XAxis dataKey="label" tick={{ fontSize: 9, fill: 'var(--text-muted)' }} tickLine={false} axisLine={{ stroke: 'var(--border)' }} angle={-35} textAnchor="end" interval={0} />
                          <YAxis tick={{ fontSize: 10, fill: 'var(--text-muted)' }} tickLine={false} axisLine={{ stroke: 'var(--border)' }} width={48} tickFormatter={v => fmt(v) ?? ''} hide={!hasDatums} />
                          <Tooltip cursor={{ fill: 'var(--bg-base)' }} content={({ active, payload }) => active && payload?.length ? <div style={tooltipStyle}><div style={{ color: 'var(--text-muted)' }}>{payload[0].payload.range}</div>{hasDatums && <div style={{ fontWeight: 600, marginTop: 4 }}>{fmt(payload[0].payload.count)} values</div>}</div> : null} />
                          <Bar dataKey="count" fill="var(--accent-hover)" radius={[4, 4, 0, 0]} isAnimationActive={false} maxBarSize={40} />
                        </BarChart>
                ...
                return (
                  <div style={{ fontSize: 40, fontWeight: 700, color: "var(--text)", display: "flex", alignItems: "baseline", gap: 10, paddingTop: 24 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px 24px", fontSize: 14 }}>
              {col.semantic_type === 'continuous' ? (
                <>
                  ...
                  <div style={{ color: "var(--text-muted)" }}>50%</div><div style={{ fontWeight: 500, textAlign: "right" }}>{col.stats_json?.median != null ? String(col.stats_json.median) : "—"}</div>
                    ...
                    <tr>
                      {columnsQ.data?.map(col => (
                        <th key={col.column_name} style={{
                          ...
                          <div style={{ fontSize: 12, color: "var(--text-muted)", fontFamily: "monospace", marginBottom: 12, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={col.data_type ?? ""}>
                            {col.data_type && col.data_type.length > 35 ? col.data_type.slice(0, 32) + "..." : col.data_type}
                          </div>
