  ...
  const avgLatency = data?.length
    ? Math.round(
        data
          ...
          .reduce((s, r) => s + (r.execution_time_ms ?? 0), 0) /
          (data.filter((r) => r.execution_time_ms).length || 1),
      )
    : 0;
              ...
              {data.map((row) => (
                <tr key={row.id}>
                  <td style={{ maxWidth: 280 }}>
                    <code
                      style={{ fontSize: 12, color: 'var(--text-muted)', whiteSpace: 'normal' }}
                    >
                      {row.raw_question.length > 70
                        ? row.raw_question.slice(0, 70) + '…'
                        : row.raw_question}
                    </code>
                  </td>
