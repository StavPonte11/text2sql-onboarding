...
import { ArrowRight, GitCompare, Minus, TrendingDown, TrendingUp } from 'lucide-react';

import { type CompareResult, orchestrationApi } from '../../api/orchestration';
...
function RunSelector({
  ...
  const available = exclude
    ? runs.filter((r) => r.id !== exclude && r.status === 'completed')
    ...
    >
      ...
      {available.map((r) => (
        <option key={r.id} value={r.id}>
          {r.id.slice(0, 8)}… · {Math.round(r.score * 100)}% ·{' '}
          {dayjs(r.created_at).format('MMM D HH:mm')} · {r.table_id.slice(0, 8)}
        </option>
      ))}
    </select>
  );
    ...
    >
      <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span
            style={{ fontSize: 18, fontWeight: 800, color: val1 >= 0.5 ? '#10b981' : '#ef4444' }}
          >
            {format(val1)}
          </span>
          <ArrowRight size={14} style={{ color: 'var(--text-muted)' }} />
          <span
            style={{ fontSize: 18, fontWeight: 800, color: val2 >= 0.5 ? '#10b981' : '#ef4444' }}
          >
            {format(val2)}
          </span>
        </div>
        ...
        {items.map((item) => (
          ...
          >
            ...
            <span style={{ fontSize: 12.5, color: 'var(--text-secondary)' }}>
              {Math.round(item.run1_score * 100)}%
            </span>
            <ArrowRight size={12} style={{ color: 'var(--text-muted)' }} />
            <span style={{ fontSize: 12.5, fontWeight: 700, color }}>
              {Math.round(item.run2_score * 100)}%
            </span>
            <span
              style={{ fontSize: 12, fontWeight: 700, color, minWidth: 40, textAlign: 'right' }}
            >
              {item.delta > 0 ? '+' : ''}
  ...
  return (
    <div>
      ...
      {isError && (
        <div
          className="card"
          style={{ textAlign: 'center', color: 'var(--status-degraded)', padding: 32 }}
        >
      ...
      {result && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <SideBySide
              label="Pass Rate"
              val1={result.run1.pass_rate}
              val2={result.run2.pass_rate}
            />
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div
              className="card"
              style={{ textAlign: 'center', border: '1px solid rgba(239,68,68,0.2)' }}
            >
            ...
            <div
              className="card"
              style={{ textAlign: 'center', border: '1px solid rgba(16,185,129,0.2)' }}
            >
