...
import {
  type EvalSchedule,
  type EvalScheduleCreate,
  orchestrationApi,
} from '../../api/orchestration';
import { EmptySlate, Spinner, StatusBadge } from '../common/EvalUI';
...
function CreateScheduleModal({
  ...
  onClose: () => void;
  onSave: (p: EvalScheduleCreate) => void;
}) {
      ...
      <div className="modal" style={{ maxWidth: 480 }}>
        ...
        <div className="form-group">
          ...
          <select
            ...
            value={isCustom ? '__custom__' : cronMode}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
          ...
          <span
            style={{
              fontSize: 13,
              color: form.enabled ? 'var(--text-primary)' : 'var(--text-muted)',
            }}
          >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            ...
            {schedule.last_run_at && (
              <span>· Last: {dayjs(schedule.last_run_at).format('MMM D, HH:mm')}</span>
            )}
          </div>
        </div>
...
export function ScheduleManager() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['eval-schedules'] });
      setShowCreate(false);
    },
  });
