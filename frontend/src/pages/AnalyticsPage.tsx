...
import { BarChart2, TrendingUp, GitCompare, Table2, ArrowUp, ArrowDown, Minus } from "lucide-react";
import { orchestrationApi } from "../api/orchestration";
import { TrendChart, RunSparkline, FailureBarChart } from "../components/monitoring/TrendChart";
import { ComparisonView } from "../components/monitoring/ComparisonView";
import { StatusBadge, ScoreBar, Spinner, SectionHeader, EmptySlate, MetricCard } from "../components/common/EvalUI";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <SectionHeader title="Score & Pass Rate Trend" sub={`Last ${days} days · ${totalRuns} runs`} />
      ...
      <div className="card">
        ...
        {isLoading
          ? <div style={{ height: 240, display: "flex", alignItems: "center", justifyContent: "center" }}><Spinner size={28} /></div>
      ...
      {trends?.runs && trends.runs.length > 0 && (
        <div className="card">
          ...
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 14 }}>Last 20 runs · ● purple = score, ○ green = pass rate · dashed lines = thresholds</div>
...
function FailureTab() {
  ...
  const totalFailures = tableData.reduce((acc, t) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <SectionHeader title="Failure Analysis" sub="Aggregated failure breakdown across all tables and runs" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div className="card">
          ...
          {[
            { type: "wrong_table",     color: "#ef4444", desc: "Agent selected wrong table for the query" },
            { type: "wrong_join",      color: "#f59e0b", desc: "Incorrect JOIN condition or missing JOIN" },
            { type: "wrong_filter",    color: "#f97316", desc: "WHERE clause filters are incorrect" },
            { type: "hallucination",   color: "#a855f7", desc: "Agent referenced non-existent columns/tables" },
            { type: "execution_error", color: "#ef4444", desc: "SQL syntax or runtime execution error" },
          ].map(({ type, color, desc }) => (
      ...
      {tableData.filter(t => Object.values(t.failure_breakdown).some(v => v > 0)).map(t => (
        <div key={t.table_id} className="card">
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)" }}>{t.table_name}</div>
              <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 2 }}>
                Score: {t.latest_score ? `${Math.round(t.latest_score * 100)}%` : "—"} · {t.run_count} runs
              </div>
            </div>
...
function TablesTab() {
  ...
  if (!tableData.length) return <EmptySlate icon={<Table2 size={36} />} title="No table data yet" sub="Run evaluations to see per-table analytics" />;
          ...
          <tbody>
            {tableData.map((t, i) => (
              <tr key={t.table_id}>
                ...
                <td>
                  <div style={{ fontSize: 13.5, fontWeight: 600, color: "var(--text-primary)" }}>{t.table_name}</div>
                  <div style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "monospace" }}>{t.table_id.slice(0, 12)}…</div>
                </td>
                ...
                <td style={{ width: 150 }}>
                  {t.avg_score !== null ? <ScoreBar score={t.avg_score} height={5} showLabel /> : "—"}
                </td>
                <td>
                  {t.pass_rate !== null ? (
                    <span style={{
    ...
    <div className="page">
      <div className="page__header">
        <div>
          ...
          <p className="page__subtitle">Deep analysis · regression detection · table performance · run comparison</p>
        </div>
      </div>
