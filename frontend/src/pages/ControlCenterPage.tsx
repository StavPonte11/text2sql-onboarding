...
// ── Failing table row ──────────────────────────────────────────────────────────
function FailingTableRow({
  table,
}: {
  table: { table_id: string; table_name: string; avg_score: number; failure_rate: number };
}) {
  return (
    <div
      style={{
    ...
    <div className="page">
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
              You do not have permission to access the admin panel. Contact your administrator to
              request access.
            </div>
          </div>
          <button
      ...
      <div className="page__header">
        <div>
          ...
          <p className="page__subtitle">
            Platform-wide evaluation monitoring · {health?.production_tables ?? 0} production tables
            · {health?.total_runs_today ?? 0} runs today
          </p>
        </div>
      ...
      >
        ...
        <MetricCard
          ...
          color={
            health?.critical_alerts ? '#ef4444' : health?.active_alerts ? '#f59e0b' : '#10b981'
          }
          icon={<AlertTriangle size={16} />}
          sub={
            health?.critical_alerts ? `${health.critical_alerts} critical` : 'No critical alerts'
          }
        />
        <MetricCard
          ...
          sub={
            health?.last_evaluation
              ? dayjs(health.last_evaluation).format('MMM D, HH:mm')
              : 'No runs yet'
          }
        />
      </div>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          {!health?.recent_runs?.length ? (
            <EmptySlate
              ...
              title="No runs yet"
              sub="Trigger an evaluation to see history"
            />
          ) : (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          >
            ...
            >
              <Bell
                size={14}
                style={{ color: alerts.length > 0 ? '#ef4444' : 'var(--text-muted)' }}
              />
              Alerts Panel
            </div>
            {alerts.length > 0 && (
          ...
          >
            {!alerts.length ? (
              <EmptySlate
                ...
                sub="System is running smoothly"
              />
            ) : (
              alerts.map((a) => (
                <AlertBanner
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          {!health?.top_failing_tables?.length ? (
            <EmptySlate
              icon={<CheckCircle size={28} />}
              title="All tables healthy"
              sub="No tables below score threshold"
            />
          ) : (
