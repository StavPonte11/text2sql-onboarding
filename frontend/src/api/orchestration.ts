...
export interface SystemHealth {
  ...
  top_failing_tables: { table_id: string; table_name: string; avg_score: number; failure_rate: number }[];
  recent_runs: {
    run_id: string; table_id: string; score: number; pass_rate: number;
    status: string; triggered_by: string; created_at: string; regression_detected: boolean;
  }[];
...
export const orchestrationApi = {
  ...
  triggerRun: (table_ids: string[], triggered_by = "user") =>
    api.post<EvalRunFull[]>("/evaluations/run", table_ids, { params: { triggered_by } }).then(r => r.data),
  ...
  // Schedules
  listSchedules: () =>
    api.get<EvalSchedule[]>("/evaluations/schedules").then(r => r.data),
  ...
  deleteSchedule: (id: string) =>
    api.delete(`/evaluations/schedules/${id}`),

  // Analytics
  getTrends: (days = 30, table_id?: string) =>
    api.get<{ runs: TrendPoint[]; daily: DailyTrend[]; total_runs: number }>(
  ...
  listAlerts: (acknowledged?: boolean, limit = 50) =>
    api.get<EvalAlert[]>("/evaluations/alerts", { params: { acknowledged, limit } }).then(r => r.data),
