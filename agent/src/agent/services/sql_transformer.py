        ...
        try:
            ...
            def transform_node(node: exp.Expression) -> exp.Expression:
                ...
                if isinstance(node, exp.In):
                    ...
                    current_vals = [extract_literal_val(node.args.get("low")), extract_literal_val(node.args.get("high"))]
                ...
                for tf in plan.enrichment_details:
                    if tf.column.lower() != col_name:
                        continue
                    if normalize_op(node_op) != normalize_op(tf.old_operator):
                        continue
                ...
                for val in current_vals:
                    ...
                    if matched_tf:
                        ...
                        # If we have any change (meaning this column is being enriched), we drop it.
                ...
                def build_expr(op_str: str, lhs: exp.Expression, refined_vals: List[str]) -> exp.Expression:
                    op_clean = normalize_op(op_str)
                    if op_clean == "IN":
                        return exp.In(this=lhs, expressions=[make_literal(v) for v in refined_vals])
                ...
                return build_expr(target_operator, node.this, refined_values)
